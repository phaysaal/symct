/*
 * wolfSSL ECDSA Key Generation Benchmark with RNG Interception
 */
...
void warmup(void) {
    ...
    key = malloc(sizeof(ecc_key) + 1024);
    int ret;
    ...
    /* Generate ECC key (P-256 / secp256r1) - 32 bytes = 256 bits */
    ret = wc_ecc_make_key(rng, 32, key);
    if (ret != 0) {
        fprintf(stderr, "wc_ecc_make_key failed in warmup: %d (%s)\n",
...
int tester_main(unsigned char *out) {
  /* WC_RNG *rng = malloc(sizeof(WC_RNG) + 1024);
     ecc_key *key = malloc(sizeof(ecc_key) + 1024); */
    int out_len = 0;
    int ret;
    ...
    /* Generate ECC key - deterministic with replayed RNG */
    ret = wc_ecc_make_key(rng, 32, key);
    if (ret != 0) {
        fprintf(stderr, "wc_ecc_make_key failed in tester_main: %d (%s)\n",
                ret, wc_GetErrorString(ret));
        wc_ecc_free(key);
        wc_FreeRng(rng);
        free(key);
    ...
    word32 outSz = 256;
    ret = wc_ecc_export_private_only(key, out, &outSz);
    if (ret == 0) {
        out_len = outSz;
    } else {
        fprintf(stderr, "wc_ecc_export_private_only failed: %d\n", ret);
    }
