...
int warmup(const unsigned char *in, int inlen, unsigned char *out) {
    ...
    //byte der[4096]; /* big enough buffer for typical PEM -> DER conversion */

    /* init rng and rsa structure */
    wc_InitRng(&rng);
    wc_InitRsaKey(&rsa, NULL);

    /* Convert PEM to DER (wolfSSL helper). PRIVATE_KEY_PEM and _LEN come from inputs */
    
    /* derLen = wc_PemToDer((const byte*)PRIVATE_KEY_PEM,
                             (long)PRIVATE_KEY_PEM_len,
                             RSA_PRIVATEKEY_TYPE, &derBuf, NULL, NULL, NULL);
    if (derLen < 0) {
      fprintf(stderr, "wc_PemToDer failed: %d\n", derLen);
      return -1;
    }
    ...
    int outlen = wc_RsaPrivateDecrypt(in, inlen,
                                     out, MAX_OUTPUT_LEN,
                                     &rsa);
